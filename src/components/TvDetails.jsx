  ...
  return (
    ...
    >
      ...
      <div className="px-4 sm:px-8 pt-8">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 mt-4">
              ...
              <span className="text-white/60 text-sm">{info.detail.first_air_date}</span>
              <span className="text-white/30">·</span>
        ...
        {/* ── Part 4: Seasons ── */}
        <SectionDivider title="Seasons & Episodes" />
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          {info.detail.seasons.map((s, i) => (
            <div key={i} className="group shrink-0 w-28 sm:w-32 flex flex-col gap-2">
              <div className="w-full aspect-[2/3] rounded-xl overflow-hidden bg-[#1c1c1c] border border-white/5">
                <img
                  className="w-full h-full object-cover"
                  src={s.poster_path ? `https://image.tmdb.org/t/p/w185/${s.poster_path}` : NoImage}
                  alt={s.name}
                />
              </div>
              <p className="text-white text-xs font-semibold leading-tight text-center line-clamp-2">{s.name}</p>
              {s.episode_count && (
                <p className="text-white/30 text-[10px] text-center">{s.episode_count} eps</p>
              )}
            </div>
          ))}
        </div>
